CONTAINER=simu_throughputXupRatio_JIT-off
SIMUPATHFILE=simuDataPath_throughputXupRatio_JIT-off.txt
		...
		do
	    	    ...
	    	    do 
					...
					do
						...
						java -Djava.compiler=NONE -cp bin contention.benchmark.Test -b linkedlists.lockbased.$ALG -W 0 -d 2000 -t $THREADS -u $UPDATE_RATIO -i $LIST_SIZE -r $LIST_RANGE > $BASEPLOTPATH/$CONTAINER/update_compare_$FILE/$ALG/$LIST_SIZE/$THREADS"_threads"/$ALG"_"$LIST_SIZE"_"$UPDATE_RATIO"_"$THREADS".txt"
